...
#include "textformat.h"    // 纯本地化映射函数（从 MainWindow 提取，便于 QtTest 单测）
...
{
    return TextFormat::text(en, zh, useChinese);
}
...
{
    return TextFormat::qualityLevelText(level, useChinese);
}

QString MainWindow::SemanticTypeText(const QString &type) const
{
    return TextFormat::semanticTypeText(type, useChinese);
}

QString MainWindow::RoleHintText(const QString &role) const
{
    return TextFormat::roleHintText(role, useChinese);
}

QString MainWindow::DirectionText(const QString &direction) const
{
    return TextFormat::directionText(direction, useChinese);
}

QString MainWindow::ToolTraceText(const QString &step) const
{
    return TextFormat::toolTraceText(step, useChinese);
}
...
{
    return TextFormat::limitationText(value, useChinese);
}
